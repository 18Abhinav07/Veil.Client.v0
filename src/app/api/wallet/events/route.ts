...
import { getLatestWalletEventId, listWalletEvents } from "@/lib/server/walletRepository";
...
export async function GET(request: Request) {
  ...
  if (!lastEventId) {
    lastEventId = await getLatestWalletEventId(getPgPool(), { userId });
  }
  let stopStream = () => {};
  ...
  const stream = new ReadableStream<Uint8Array>({
    start(controller) {
      ...
      async function tick() {
        if (closed) return;
        try {
          const events = await listWalletEvents(getPgPool(), {
            userId,
            afterEventId: lastEventId,
            limit: 100,
          });
          if (events.length === 0) {
            if (!enqueue({ event: "heartbeat", data: { now: new Date().toISOString() } })) {
              return;
            }
          } else {
      ...
      enqueue({
        event: "connected",
        data: { ok: true, streams: ["contact", "request", "spend_job"] },
      });
      void tick();
    },
    cancel() {
